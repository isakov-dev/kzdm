...
{
	$('#ajax_configurator_block').css("opacity",0.3);
	if(!!this.timer)
	{
		clearTimeout(this.timer);
	}
			...
			if (arItem.VALUES.hasOwnProperty(i))
			{
				...
				if (!!control)
				{
					...
					if (value.DISABLED)
					{
							...
							if (label)
								BX.addClass(label, 'filter-disabled');
							else
								BX.addClass(control.parentNode, 'filter-disabled');
					}
					else
					{
							BX.adjust(control, {props: {disabled: false}});
							if (label)
								BX.removeClass(label, 'filter-disabled');
							else
								BX.removeClass(control.parentNode, 'filter-disabled');
					}
...
JCSmartFilter.prototype.postHandler = function (result, fromCache)
{
	...
	if (!!result && !!result.ITEMS)
	{
		...
		{	
			...
			var thisUrl=urldecode(result.FILTER_AJAX_URL)+"&AJAX_FILTER=Y";
			
			console.log(thisUrl);
			$('#ajax_configurator_block').load(thisUrl+" #ajax_configurator_block >*",function(data){
				console.log(data);
				$('#ajax_configurator_block').css("opacity",1);
			})
